// Variant that reads {ability_name, parameters} from a JSON file
const fs = require('fs');
const { spawn } = require('child_process');

const argsFile = process.argv[2];
if (!argsFile) { console.error('Usage: node mcp_call_file.cjs <argsFile.json>'); process.exit(2); }
...
(async () => {
  try {
    ...
    // mcp-adapter-execute-ability expects: { ability_name, parameters }
