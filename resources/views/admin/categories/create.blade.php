        ...
        <div class="form-grid">
                        ...
                        <div class="field-section">
                            ...
                            <div class="field">
                                ...
                                <input id="name" type="text" name="name" value="{{ old('name') }}"
                                       ...
                                       placeholder="e.g. Healthcare & Medical, IT & Communications, Sales"
                                       required oninput="updateSlugPreview(this.value)">
                                @error('name')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                <div class="slug-preview" id="slugPreview" style="display:none;">
                                    <span class="lbl">URL slug</span>
                                    <strong id="slugValue">—</strong>
                                </div>
                                <p class="help-text">The URL slug is auto-generated from the name — e.g. <code>"Healthcare &amp; Medical"</code> becomes <code>healthcare-medical</code>.</p>
                            </div>
                        </div>

                        {{-- Description --}}
                        <div class="field-section">
                            <p class="section-label">SEO &amp; Description <span style="color:#9ca3af; font-weight:500; font-size:11px; letter-spacing:0.4px; text-transform:none; margin-left:4px;">(optional)</span></p>
                            <div class="field">
                                <label for="description" style="display:flex;align-items:baseline;">
                                    ...
                                    <button type="button"
                                            ...
                                            title="Fill in Name first, then click">
                                        <i class="bi bi-stars"></i> Generate with AI
                                    </button>
                                </label>
                                <textarea id="description" name="description" rows="6"
                                          class="@error('description') is-invalid @enderror"
                                          placeholder="A short description of what kinds of jobs fit in this category. Visible on the public category landing page and used for SEO.">{{ old('description') }}</textarea>
                                @error('description')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                <p class="help-text">2–3 sentences works best for SEO. Mention typical roles, industries, or skills candidates should expect.</p>
                            </div>
                        </div>
                    </div>
                    <div class="form-foot">
                        <a href="{{ route('admin.categories.index') }}" class="btn btn-outline"><i class="bi bi-x-lg"></i> Cancel</a>
                        <button type="submit" class="btn btn-primary"><i class="bi bi-check2"></i> Create Category</button>
                    </div>
                </div>
            </div>
            ...
            <aside>
                <div class="info-card">
                    ...
                    <h4>Categories organise your jobs</h4>
                    <p>Categories help job seekers filter the jobs that match their field. Each job belongs to one category.</p>
                    <ul>
                        <li><i class="bi bi-check-circle"></i><span>Use clear, common names — like "Healthcare", "IT", "Sales".</span></li>
                        <li><i class="bi bi-check-circle"></i><span>The slug is auto-generated from the name and used in URLs.</span></li>
                        <li><i class="bi bi-check-circle"></i><span>Description appears on the public category landing page.</span></li>
                        <li><i class="bi bi-check-circle"></i><span>Deleting a category will unassign jobs in it — the jobs won't be deleted.</span></li>
                    </ul>
                </div>
            </aside>
        </div>
...
<script>
    function updateSlugPreview(value) {
        ...
        const slug = document.getElementById('slugValue');
        const v = (value || '').toString().toLowerCase()
            .replace(/[^\w\s-]/g, '').replace(/\s+/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '');
        if (v) { slug.textContent = v; preview.style.display = 'inline-block'; }
