...
<style>
    ...
    .field input:focus { outline: none; border-color: #0a0a0a; box-shadow: 0 0 0 3px rgba(10,10,10,.10); }
...
<div class="loc-form-wrap">
    ...
    <form action="{{ route('admin.locations.update', $location) }}" method="POST">
        ...
        <div class="form-grid">
            <div>
                <div class="panel">
                    ...
                    <div class="panel-body">
                        <div class="field">
                            <label for="name">State / City Name <span class="req">*</span></label>
                        ...
                        <div class="row-2">
                            <div class="field" style="margin: 0;">
                                ...
                                <input id="area" type="text" name="area" value="{{ old('area', $location->area) }}"
                                       class="@error('area') is-invalid @enderror">
                                @error('area')<div class="invalid-feedback">{{ $message }}</div>@enderror
                            </div>
                            <div class="field" style="margin: 0;">
                                <label for="country">Country</label>
                                <input id="country" type="text" name="country" value="{{ old('country', $location->country) }}"
                        ...
                        <div class="field">
                            ...
                            <input id="postal_code" type="text" name="postal_code" value="{{ old('postal_code', $location->postal_code) }}"
                                   class="@error('postal_code') is-invalid @enderror" style="max-width: 240px;">
                            @error('postal_code')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>
                    </div>
                    <div class="form-foot">
                        <a href="{{ route('admin.locations.index') }}" class="btn btn-outline"><i class="bi bi-x-lg"></i> Cancel</a>
                        <button type="submit" class="btn btn-primary"><i class="bi bi-check2"></i> Update Location</button>
                    </div>
                </div>
            </div>
            ...
            <aside>
                <div class="meta-card">
                    <h4>Location Info</h4>
                    <div class="meta-row"><span class="lbl">ID</span><span class="val">#{{ $location->id }}</span></div>
                    <div class="meta-row"><span class="lbl">Total Jobs</span><span class="val">{{ number_format($location->jobs()->count()) }}</span></div>
                    <div class="meta-row"><span class="lbl">Created</span><span class="val">{{ optional($location->created_at)->format('M d, Y') ?? '—' }}</span></div>
                    <div class="meta-row"><span class="lbl">Last Updated</span><span class="val">{{ optional($location->updated_at)->format('M d, Y') ?? '—' }}</span></div>
                </div>
            </aside>
        </div>
    </form>
    ...
    <div class="panel" style="margin-top: 22px;">
        ...
        <div class="panel-body" style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:14px;">
            <div>
                <div style="font-weight:700; color:#0f172a; font-size:14.5px; margin-bottom:4px;">Delete this location</div>
                <div style="font-size:13px; color:#6b7280; max-width:520px;">Jobs at this location will lose their location assignment — they won't be deleted. This action cannot be undone.</div>
            </div>
            <form action="{{ route('admin.locations.destroy', $location) }}" method="POST"
                  onsubmit="return confirm('Permanently delete this location? Jobs at it will be unassigned. This cannot be undone.');">
                @csrf
                @method('DELETE')
